...
CROPS = {
    ...
    "TOMATO": (50, 8, 11, 1, 4), "STRAWBERRY": (100, 10, 16, 2, 4),
    "MELON": (80, 10, 12, 0, 6),
}
...
def _price(item, stock, overrides):
    ...
    if stock < anchor:
        ...
        return max(1., base + amount / _shape(shape, throughput, throughput))
...
def _move_actions(start, end):
    ...
    return ([["EAST" if a > x else "WEST"]] * abs(a - x)
            + [["SOUTH" if b > y else "NORTH"]] * abs(b - y))
...
def _crop_events(crop, planted, day, horizon, tile=None, fertilize=False):
    ...
    if not interval:
        ...
        _add_event(events, crop, min(1, horizon), tile.get("yield_units", 0) if tile else 0, horizon)
        for a in range(first, last + 1, interval):
            if a < age or (tile and a == age):
                continue
            when = a - age
            if when <= horizon:
                _add_event(events, crop, min(when + 1, horizon), 2 if fertilize else 1, horizon)
    return events
...
def _animal_events(animal, placed, day, horizon, tile=None, care=True):
    ...
    _add_event(events, product, min(1, horizon), tile.get("yield_units", 0) if tile else 0, horizon)
    for d in range(horizon + 1):
        if d > 0 or (tile and tile.get("fertilizer_available")):
            _add_event(events, "FERTILIZER", min(d + 1, horizon), 1, horizon)
        if d < horizon:
            _add_event(events, "WHEAT", d, -1 if care else -.5, horizon)
        a = age + d + 1
        if d < horizon and a >= first and (a - first) % interval == 0:
            _add_event(events, product, min(d + 2, horizon), min(cap, 1 + pending), horizon)
            pending = 0
        if care:
            pending += 1
    return events
...
def _forecast(obs, cfg):
    ...
    for shop in shops:
        for p in SHOPS.get(shop, ()):
            known[p] += shop_tick
    net = {p: [0.] * (horizon + 1) for p in PARAMS}
        ...
        for row in farm["tiles"]:
            for tile in row:
                ...
                if tile.get("kind") == "PLANT":
                    events = _crop_events(tile["crop"], tile["planted_day"], day, horizon, tile)
                elif tile.get("animal") in ANIMALS:
                    events = _animal_events(tile["animal"], tile["placed_day"], day, horizon, tile)
                else:
                    continue
    ...
    for p in PARAMS:
        ...
        net[p][min(1, horizon)] += sum(inv.get(p, 0) for inv in private.get("inventories", []))
...
def _project(kind, day, horizon, stocks, overrides):
    if kind in CROPS:
        ...
        events = _crop_events(kind, day, day, horizon)
        labor = .7 + (2. if interval else 2.5) / max(1, duration)
    else:
    ...
    for item, d, quantity in sorted(events, key=lambda e: e[1]):
        old = cumulative.get(item, 0.)
        value = _price(item, stocks[item][d] + old + quantity * .5, overrides)
        revenue += quantity * value * .98 ** d
        cumulative[item] = old + quantity
    annuity = sum(.98 ** d for d in range(max(1, duration)))
...
def _base_jobs(obs, cfg, stocks, horizon):
    ...
    for y, row in enumerate(farm["tiles"]):
        for x, tile in enumerate(row):
            ...
            if tile == "LOCKED":
                continue
            if tile is None or tile.get("kind") == "WEED" or (tile.get("kind") in ("COOP", "PASTURE") and not tile.get("animal")):
            ...
            if tile.get("animal") in ANIMALS:
                ...
                care = horizon > 1 and output_value > max(5, prices["WHEAT"] * .55)
                ...
                if ops:
                    jobs.append(_job(pos, ops, 2000. + tile.get("yield_units", 0) * prices[product]))
                continue
            crop = tile["crop"]
            _, first, last, interval, cap = CROPS[crop]
            ...
            if ripe and not interval and age < target:
                now_qty = min(cap, tile["yield_units"] + (not tile.get("watered_today") and age >= window))
                future_qty = min(cap, now_qty + target - age)
                ahead = min(horizon, target - age + 1)
                if prices[crop] * now_qty > 1.12 * _price(crop, stocks[crop][ahead], overrides) * future_qty:
                    clear = True
            ...
            if interval and horizon > 0 and age < last:
                productions = [a for a in range(first, last + 1, interval) if age < a <= min(age + 3, age + horizon)]
                if productions and tile.get("fertilized_until_day", -1) < day:
                    value = sum(_price(crop, stocks[crop][min(horizon, a - age + 1)], overrides) for a in productions)
                    fertile = value > prices["FERTILIZER"] * 1.3 + 12 and productions[0] == age + 1
                if productions and productions[0] == age + 1 and (fertile or tile.get("fertilized_until_day", -1) >= day):
                    water = not tile.get("watered_today")
            elif not interval and not tile.get("watered_today") and window <= age <= last and tile.get("fertilized_until_day", -1) < day:
                remaining = max(0, min(last, age + 2, target) - age + 1)
                extra = min(remaining, max(0, cap - tile.get("yield_units", 1) - remaining))
            ...
            if clear:
                ...
                jobs.append(_job(pos, ops, 1500. + tile.get("yield_units", 0) * prices[crop]))
...
def _routes(jobs, spawns, sheds, hours, final):
    ...
    order = sorted(jobs, key=lambda j: (-j["value"] // 1000, -min(_distance(j["pos"], p) for p in sheds), -len(j["ops"]), j["pos"]))
    for job in order:
        best = None
        for i, route in enumerate(routes):
            for slot in range(len(route) + 1):
                ...
                if cost <= hours:
                    candidate = (cost - costs[i] + .055 * cost, i, slot, cost)
...
def _plan(obs, cfg):
    ...
    remaining = min(tpd - obs["hour"], cfg.get("episodeSteps", 720) - 1 - obs.get("step", day * tpd))
    base_inputs = _supplies(jobs)
    cash = farm["money"]
    for p in PARAMS:
        sellable = max(0, private["shed"].get(p, 0) - base_inputs.get(p, 0))
        stock = obs["market"]["inventory"][p]
        cash += sum(round(_price(p, stock + n, overrides)) for n in range(sellable)) * .98
    animal_count = sum(bool(isinstance(t, dict) and t.get("animal")) for row in farm["tiles"] for t in row)
    maintenance = sum(max(0, n - private["shed"].get(p, 0)) * (prices[p] * 1.1 + 1)
                      for p, n in base_inputs.items() if p in PARAMS)
    reserve = maintenance + 100 + animal_count * prices["WHEAT"] * 1.25
    budget, additions, buy_land = max(0., cash - reserve), [], False
    if not available and horizon >= 9 and len(farm["unlocked_quadrants"]) < 4:
        land_cost = (1000, 2000, 4000)[len(farm["unlocked_quadrants"]) - 1]
        candidates = [_project(k, day, horizon, stocks, overrides) for k in (*CROPS, *ANIMALS)]
        best = max((c[0] for c in candidates if c), default=0)
        if budget > land_cost + 1000 and best * horizon * (size // 2) ** 2 > land_cost * 2.5:
            buy_land, budget = True, budget - land_cost
            quadrant = ("NE", "SW", "SE")[len(farm["unlocked_quadrants"]) - 1]
            for y in range(size):
                for x in range(size):
                    q = ("N" if y < size // 2 else "S") + ("W" if x < size // 2 else "E")
                    if q == quadrant:
    ...
    for pos, prefix in available:
        ...
        if horizon > 0 and work < MAX_WORKERS * max(3, tpd - 6):
            for kind in (*CROPS, *ANIMALS):
                ...
                owned = private["seeds"].get(kind, 0) if kind in CROPS else private["shed"].get(kind, 0)
        ...
        if best:
            ...
            if kind in CROPS:
                ...
                ops = prefix + [["BUILD_" + ANIMALS[kind][5]], ["PLACE", kind], ["FEED"], ["CARE"]]
            jobs.append(_job(pos, ops, 1100 + max(0, score) * 2))
        elif prefix:
            jobs.append(_job(pos, prefix, 1800.))
    ...
    for count in range(1, max_workers + 1):
        ...
        routes, skipped = _routes(jobs, spawns, sheds, max(1, remaining - 3), horizon == 0)
        hire_cost = sum(FIB[:count - 1]) * cfg.get("farmHandCostMult", 1)
        loss = sum(j["value"] for j in skipped) + hire_cost
        if hire_cost > max(0., cash - maintenance) * .35 and count > 1:
            continue
        if loss < best_loss:
    ...
    for item, count in needed.items():
        ...
        if count > 0:
            purchases.append(["BUY_ANIMAL" if item in ANIMALS else "BUY_PRODUCT", item, count])
    for item, count in seeds.items():
        count -= private["seeds"].get(item, 0)
        if count > 0:
            purchases.append(["BUY_SEED", item, count])
    return {"day": day, "jobs": jobs, "needed": needed, "purchases": purchases,
            "workers": chosen, "routes": None, "stage": 0, "horizon": horizon,
            "skipped": 0, "last_step": obs.get("step", 0) - 1}
...
def _prepare_routes(plan, obs, cfg):
    ...
    hours = min(cfg.get("turnsPerDay", 24) - obs["hour"], cfg.get("episodeSteps", 720) - 1 - obs["step"])
    ...
    for job in plan["jobs"]:
        ...
        for action in job["ops"]:
            ...
            ops.append(action)
        if ops:
            jobs.append(_job(job["pos"], ops, job["value"]))
...
def agent(obs, configuration=None):
    ...
    if plan["routes"] is None:
        ...
        for _ in range(min(missing, limit - len(market))):
            market.append(["HIRE"])
        if not market or (not plan["purchases"] and plan["stage"] > 0 and (not missing or plan["stage"] >= 3)):
