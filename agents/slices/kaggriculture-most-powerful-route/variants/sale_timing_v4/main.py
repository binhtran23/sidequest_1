...
class FarmView:
    ...
    def __init__(self, observation):
        ...
        self.shed = dict(private["shed"])  # Engine contract: nonnegative integer quantities.
...
def _v224_sales_first(action):
    ...
    changed={'farmer':list(action['farmer']), 'hands':[list(c) for c in action['hands']], 'market':orders}
    return changed
...
# V227: spend fertilizer a unit already carries, on a turn the route left idle.
# Measured 2026-09-12 (experiments/season-curves-20260912): this agent acquires
# 343.8 fertilizer units a game, more than any top-5 team, and applies 67.7,
# fewer than any of them. It cannot stop selling the balance -- withholding that
# revenue starves a budgeted HIRE and collapses the route -- so the reachable
# part is the stock already in a worker's hands on a turn that was wasted.
# Own tile only, PASS only: no route step moves, no market order changes, and
# the hand count is untouched. Before day 18 the same rule loses games.
_V227_PARENT = agent
del agent
_V227_START_DAY = 18
_V227_REPORT = {'fertilize_on_pass': 0}


def _v227_apply(observation, action, day):
    farm = observation['farms'][observation['player']]
    tiles = farm['tiles']
    ...
    for unit in range(min(len(commands), len(positions))):
        ...
        if (carried and isinstance(tile, dict) and tile.get('crop')
            ...
            _V227_REPORT['fertilize_on_pass'] += 1
    action['farmer'], action['hands'] = commands[0], commands[1:]
    return action


def agent(observation, configuration=None):
    action = _V227_PARENT(observation, configuration)
    step = int(observation['step'])
    day = step // TURNS_PER_DAY
    if step >= LAST_STEP or day < _V227_START_DAY:
        return action
    return _v227_apply(observation, action, day)


agent.telemetry = _V227_PARENT.telemetry
...
Same layer as loss_upgrade_v1, composed onto route-v2-fert18 instead of
route-v1-h3. That base already carries V227, which fertilizes on any PASS turn
from day 18 without an economic test, and V227 runs first because this layer
wraps it. So `fertilizer: window` here only sees the turns V227 left alone --
in practice the redundant-WATER turns -- and applies its bonus-window and
value tests to those. The two measured gains (V227 at +203.8 coins/game, this
layer at +2,535 against route-v1) have never been combined before.
"""
...
# --- Phase 3b: advance goods whose price is already sliding ------------------
#
# The market is a ratchet. Every unit is priced off live inventory
# (`_process_market`), and the only demand is the town's: each unlocked shop
# consumes one or two units of its own products every fourth step, plus one of
# each product a day from the town centre. Nothing else removes inventory, so a
# good the shops do not cover falls and stays down. Measured on seed 930000
# against the public-state router, with no YARN_STORE in the draw, WOOL runs
# 217 -> 154 -> 1 between days 6 and 18 and is still 1 on day 30, while
# STRAWBERRY -- covered seven times over by that seed's shops -- holds 161 ->
# 237 across the same span.
#
# For a good on that slide a later sale is strictly a worse sale, so moving it
# earlier costs nothing: the quantity is unchanged and only the turn moves. The
# layer therefore reuses the inherited sale window, which already reserves a
# future planned SELL against today's stock and subtracts it again when the turn
# arrives. It never withholds -- the tape budgets its hires against every coin,
# and starving one collapses the season.
#
# Two goods are deliberately left alone. WHEAT is animal feed the tape buys back,
# and FERTILIZER is consumed by the two fertilizer layers above; selling either
# out from under them is withholding by another name.

_DECAY_SETTINGS = {"start_step": 288, "horizon": 6, "trigger": 0.97, "lookback": 24}
_DECAY_HISTORY = {}
_DECAY_RESERVED = ('WHEAT', 'FERTILIZER')
_DECAY_PARENT_RESERVE = reserve_sales


def _decay_track(observation):
    """Record this turn's prices per player; a step 0 means a fresh game."""
    player = int(observation['player'])
    if int(observation['step']) == 0:
        for key in [k for k in _DECAY_HISTORY if k[0] == player]:
            del _DECAY_HISTORY[key]
    lookback = int(_DECAY_SETTINGS.get('lookback', 24))
    for item, price in observation['market']['prices'].items():
        history = _DECAY_HISTORY.setdefault((player, item), [])
        history.append(price)
        if len(history) > lookback + 1:
            del history[:len(history) - lookback - 1]


def _decay_falling(player):
    """Goods worth strictly less than they were a day ago, and still worth selling."""
    lookback = int(_DECAY_SETTINGS.get('lookback', 24))
    trigger = float(_DECAY_SETTINGS.get('trigger', 0.97))
    falling = set()
    for (owner, item), history in _DECAY_HISTORY.items():
        if owner != player or item in _DECAY_RESERVED or len(history) <= lookback:
            continue
        # Below 2 the native pass declines the good anyway, and at the floor
        # there is no longer any slide left to get in front of.
        if history[-1] >= 2 and history[-1] < history[0] * trigger:
            falling.add(item)
    return falling


def reserve_sales(action, view, state, tape, step):
    """Run the inherited window, then a second pass over the sliding goods only."""
    _DECAY_PARENT_RESERVE(action, view, state, tape, step)
    observation = _LOSS_OBSERVATION
    if observation is None or step < int(_DECAY_SETTINGS.get('start_step', 288)):
        return
    if step >= LAST_STEP or len(action['market']) >= MAX_ORDERS:
        return
    booked = {o[1] for o in action['market'] if len(o) > 1 and o[0] in ('SELL', 'BUY_PRODUCT')}
    # The native pass has already projected this turn's field work onto the shed.
    stock = getattr(view, '_sale_stock', None) or {}
    targets = {item for item in _decay_falling(int(observation['player']))
               if item not in booked and stock.get(item, 0) > 0}
    if not targets:
        return
    global SALE_HORIZON
    prices, horizon = view.prices, SALE_HORIZON
    extended = int(_DECAY_SETTINGS.get('horizon', 6))
    # Masking the price to zero is how the inherited layer confines a pass to a
    # subset: the native loop skips any good priced under 2.
    view.prices = {p: (v if p in targets else 0) for p, v in prices.items()}
    SALE_HORIZON = extended
    start = len(_SUBMISSION_SALE_EVENTS)
    try:
        _LOSS_NATIVE_RESERVE(action, view, state, tape, step)
        for event in _SUBMISSION_SALE_EVENTS[start:]:
            event['horizon'] = extended
            event['reason'] = 'price_decay'
    finally:
        view.prices = prices
        SALE_HORIZON = horizon


_DECAY_PARENT_AGENT = agent


def agent(observation, configuration=None):
    _decay_track(observation)
    return _DECAY_PARENT_AGENT(observation, configuration)
